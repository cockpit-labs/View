  ...
  <div class="target-selector" :disabled="only1Target">
    ...
    <div ref="targetList" :class="['target-list', {open: modalOpened}]" v-show="expanded || modalOpened">
      ...
      <div
        ...
        @click="setSelectedTarget(currentNode)"
      >
...
export default {
  ...
  computed: {
    ...
    selectedTarget () {
      if (this.selectedTargetId) {
        return Target.query().whereId(this.selectedTargetId).with('children').with('parent').first()
      } else {
        return null
      }
    },
    ...
    currentNode () {
      if (this.currentNodeTargetId === null && this.selectedTarget && !this.only1Target) {
        ...
        if (this.selectedTarget.children.length > 0) {
          return Target.query().whereId(this.selectedTarget.id).with('children.children').with('parent').first()
        } else {
          return Target.query().whereId(this.selectedTarget.parent.id).with('children.children').with('parent').first()
        }
      } else if (this.currentNodeTargetId) {
        // Afficher le noeud demandé
        return Target.query().whereId(this.currentNodeTargetId).with('children.children').with('parent').first()
      } else {
        // Afficher le noeud racine
        return Target.query().where('parentId', null).with('children.children').first()
      }
    },
  ...
  methods: {
    ...
    setSelectedTarget (target) {
      this.closeList()
      this.$store.commit('setSelectedTargetId', target.id)
      this.$emit('selectTarget', target.id)
    },
    reset () {
      this.$store.commit('setSelectedTargetId', null)
